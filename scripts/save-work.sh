...
if [[ -z "$current_branch" ]]; then
  echo "Refusing to save work from detached HEAD."
  exit 1
fi

if [[ "$current_branch" == "main" && "$ALLOW_MAIN" -ne 1 ]]; then
  if [[ -z "$TARGET_BRANCH" ]]; then
    TARGET_BRANCH="codex/$(slugify "$COMMIT_MESSAGE")-$(/bin/date '+%Y%m%d-%H%M%S')"
  fi
  ...
  echo "Saving directly on main because --allow-main was provided."
...
current_branch=$(/usr/bin/git branch --show-current)
echo "Save branch: $current_branch"
