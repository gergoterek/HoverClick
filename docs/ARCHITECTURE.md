...
2. Performs the ultra-fast menu bar check via `pointIsInScreenMenuBarArea:` — this fires at the earliest safe point, immediately after acquiring the raw CGEvent point and before any `NSWorkspace` IPC, before any `NSMutableDictionary` allocation, and before any diagnostic string formatting. Converts the CGEvent point to AppKit coordinates, then for the matched screen computes the effective menu bar strip height as `MAX(NSScreen.visibleFrame-derived height, NSStatusBar.systemStatusBar.thickness)`. The `thickness` fallback is necessary when "Automatically hide and show the menu bar" is enabled: in that state `NSScreen.visibleFrame == NSScreen.frame` for every screen so the visibleFrame-derived height is zero and visibleFrame-only detection silently fails. If the point is within the top strip, the original event is returned unchanged immediately. In the fast path only three cheap writes occur: `_totalMenuStatusUISkips++`, `_lastMenuBarFastPassthroughSequence/Time/Point` (primitive assignments). No `beginRecentDecisionWithTrigger:` is called (no `[NSWorkspace sharedWorkspace].frontmostApplication` IPC, no 20-entry NSDictionary), no diagnostics strings are formatted before returning, no focus-decision records are created, and no focus attempt is made. The full check geometry (CGEvent point, AppKit point, screen frame, visibleFrame, heights, boundary, result) is still stored in `_lastMenuBarCheckDescription` and appears as `Last menu-bar/top-strip check:` in Copy Diagnostics Summary. The last ultra-fast passthrough (left/right, sequence, timestamp, raw point) is stored as primitives and formatted lazily at summary-build time, appearing as `Last ultra-fast menu-bar passthrough:` in Copy Diagnostics Summary. A `menu-bar-check` diagnostic log line is also emitted for NSLog.
