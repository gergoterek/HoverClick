...
2. Checks whether the click point is within the menu bar area of any connected screen via `pointIsInScreenMenuBarArea:`. Converts the CGEvent point to AppKit coordinates, then for the matched screen computes the effective menu bar strip height as `MAX(NSScreen.visibleFrame-derived height, NSStatusBar.systemStatusBar.thickness)`. The `thickness` fallback is necessary when "Automatically hide and show the menu bar" is enabled: in that state `NSScreen.visibleFrame == NSScreen.frame` for every screen so the visibleFrame-derived height is zero and visibleFrame-only detection silently fails. If the point is within the top strip, the focus attempt is skipped immediately and the original event is returned unchanged. The full check result (CGEvent point, AppKit point, screen frame, visibleFrame, heights, boundary, result, and whether bypassed) is written to `_lastMenuBarCheckDescription` on every click and appears as `Last menu-bar/top-strip check:` in Copy Diagnostics Summary. A `menu-bar-check` diagnostic log line is also emitted for NSLog.
