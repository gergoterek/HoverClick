...
The status menu starts with one non-clickable custom header row: `HoverClick`. The label uses a disabled text color; the custom view uses a compact 24 pt height, starts at indentation level 0, and uses a shared 14 pt horizontal padding constant. Version/build are intentionally not shown in the header or Diagnostics submenu so release-specific UI text cannot go stale; `About HoverClick...` is the single UI surface for version/build. Header and status-item tooltips use stable, release-independent wording.
