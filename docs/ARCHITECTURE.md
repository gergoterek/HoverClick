...
The status menu starts with one non-clickable custom header row containing a small green status dot, `HoverClick is running`, and a right-aligned `v<short-version>` label. The labels use disabled text color; the custom view uses a compact 24 pt height, starts at indentation level 0, and uses shared leading/trailing geometry so the status dot, status text, and version label line up with the same visual columns used by the menu rows. The visible header version reads from `CFBundleShortVersionString` through the runtime version helper, with a generic fallback only for malformed bundle metadata. Build number is intentionally not shown in the header or Diagnostics submenu; bottom `About` remains the single UI surface for full version/build. Header and status-item tooltips use stable, release-independent wording.

Feature toggles use stable titles rather than appending `On` or `Off` to the title. The top-level menu is grouped with smaller subtle section headers: Functions, Access, Info, and Updates, followed by a bottom separator with `About` above `Quit` with Cmd+Q. Functions contains Left Click Focus and Right Click Focus. Access contains a `Permissions` submenu with Accessibility status, Launch at Login, `Refresh Status`, and `Accessibility Settings`. Info contains `Guide`, `Help`, and `Diagnostics` submenus. Guide is the visible help fallback for native in-menu hover tooltips, which AppKit does not display on HoverClick's custom `NSView` menu rows (NSMenu's tracking loop never drives `NSView` tooltip display, and a custom `item.view` bypasses `NSMenuItem.toolTip`); it holds short word-wrapped explanations for Left Click Focus, Right Click Focus, Bypass Key, Excluded Apps, Launch at Login, Permissions, Verbose Mode, and Copy Summary, built by `HoverClickCreateQuickHelpItem` as disabled informational rows (plain `NSView`, no tracking area/highlight/event loop, skipped by the highlight-reset pass) fixed to the menu content width so descriptions wrap instead of widening the menu. Help opens GitHub, Contact, Release Notes, and safe uninstall instructions; Diagnostics keeps `Copy Summary` and `Verbose Mode`; Updates keeps Sparkle's manual check and automatic-check toggle. The updater toggle writes through Sparkle's `automaticallyChecksForUpdates` setting, while `SUAutomaticallyUpdate` and `SUAllowsAutomaticUpdates` remain false so automatic download/install stays disabled. Submenu parent items do not carry tooltips, while child status/action items keep specific native `NSMenuItem` tooltip/help text. Non-closing in-menu rows for focus toggles, permission refresh, Launch at Login, diagnostics copy, verbose diagnostics, and automatic update checks use small custom `NSView` rows backed by the original `NSMenuItem` sender/action, with SF Symbol images on the left and state indicators on the right. Custom header/control rows use named leading inset, icon spacing, and right accessory constants so section labels, left icons, title text, right-side indicators, and the header version share predictable visual columns anchored to the plain `About`/`Quit` rows. Other rows use normal `NSMenuItem.image` with SF Symbols where available instead of text icon prefixes or leading title padding, avoiding the previous `icon + space + label` appearance. `About` and `Quit` are plain native rows with no icons; `About` shows a small native informational alert with the current version, build, bundle identifier, and one-line description. GitHub, Contact, and Release Notes use explicit user-click link actions; Uninstall only shows instructions and deletes nothing. Native AppKit menus may still reserve a shared gutter, and HoverClick avoids fragile all-custom menu workarounds outside the rows that need non-closing behavior. Technical runtime details such as click detection state, updater state, and last handled action are not persistent diagnostic rows; they are available through `Diagnostics` > `Copy Summary`. Because native menu tooltips proved insufficient on custom rows, discoverability is provided by the static `Guide` submenu above rather than by custom tracking loops, focus-stealing help windows, hover timing controls, or hover event monitors, all of which remain intentionally unused.
...
**Menu:** The `Excluded Apps` submenu (Functions section, after Bypass Key, before the section separator) has two sections separated by a subsection header and optional separator. When Maccy is installed/resolvable an `Auto-added` section header appears first, followed by a non-interactive, non-removable row (`HoverClickCreateAutomaticEntryItem`) showing a `lock` SF Symbol icon on the left and the Maccy app name — no right-side accessory text, no tracking area; icon and title both rendered in `secondaryLabelColor` so the row reads as managed/locked but remains clearly readable. Then an `Added exclusions` section header, followed by user-added apps by friendly display name (each clickable to remove via `removeExcludedApp:`) sorted A–Z, or a disabled `No added exclusions` row when the user list is empty. A separator and `Add App Exclusion...` bottom the submenu. The built-in Maccy bypass is never persisted into `excludedAppBundleIDs`; it remains unconditionally active regardless of the visible row. Stored bundle IDs are resolved to friendly names with `displayNameForExcludedBundleID:` (via `NSWorkspace URLForApplicationWithBundleIdentifier:`), falling back to the raw bundle ID when the app is not installed. Section headers inside the submenu use `HoverClickCreateSubmenuSectionHeader` (width-aware, `secondaryLabelColor`) so they do not force the submenu wider than its calculated content width.

**App selector (`Add App Exclusion...`):** `chooseExcludedApp:` presents a dedicated app list selector — an `NSAlert` with an `NSPopUpButton` whose items are installed applications (display name + icon), sorted alphabetically and deduplicated by bundle ID. This is a list picker in the LinearMouse "Configure for" spirit, not a Finder/`NSOpenPanel` file browser. `installedApplicationsForSelector` enumerates apps from standard locations only — `/Applications`, `/Applications/Utilities`, `/System/Applications`, `/System/Applications/Utilities`, and `~/Applications` — with a shallow, non-recursive `NSFileManager contentsOfDirectoryAtPath:` scan (no full-disk scan, no deep recursion). For each `.app`: display name from `CFBundleDisplayName`/`CFBundleName`/file name, bundle ID from `NSBundle bundleWithURL:`, and icon from `NSWorkspace iconForFile:` (copied and sized to the menu image size). Apps without a bundle ID are skipped.

Selecting an app funnels its bundle ID into the shared `addExcludedBundleID:`, which trims whitespace, rejects missing IDs and duplicates, and treats the built-in Maccy bundle ID as already-handled (a friendly message, not an added entry), shown via `showExcludedAppsAlertWithTitle:message:`. It then saves to `NSUserDefaults`, logs, and rebuilds the submenu. No app restart is required.

The earlier `NSOpenPanel`-based `Choose Application...`, the manual `Add by Bundle ID...` dialog, the `HoverClickEditableTextField` subclass, and the `panel:shouldEnableURL:` delegate were removed: the normal user path no longer requires browsing files or typing bundle IDs.
