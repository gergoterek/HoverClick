...
- Multi-monitor menu bar pass-through: before any AX lookup, HoverClick checks whether the click point falls within the menu bar area of any connected screen. The menu bar strip height is determined as the maximum of `NSScreen.visibleFrame`-reported height and `NSStatusBar.systemStatusBar.thickness`. Using `thickness` as the definitive fallback ensures the check works even when "Automatically hide and show the menu bar" is enabled, a configuration where `NSScreen.visibleFrame` equals `NSScreen.frame` for all screens and the `visibleFrame`-only approach silently returns NO for every click. If the point is in the menu bar strip, the focus attempt is skipped entirely and the original event is returned unchanged. The result of the check (including CGEvent point, converted AppKit point, matched screen frame/visibleFrame, computed heights, boundary, and YES/NO result) is recorded in `Last menu-bar/top-strip check:` in Copy Diagnostics Summary after every click, making it visible without needing to read NSLog. Menu-bar-bypassed clicks now also appear in the Recent non-menu mouse-down decisions history with result `skipped: click in screen menu bar area`.
