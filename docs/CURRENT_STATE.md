...
- Maccy automatic row (#031 / polished on #033): `buildExcludedAppsSubmenu` checks `URLForApplicationWithBundleIdentifier:HoverClickMaccyBundleID` on each menu open. If Maccy is installed, an `Automatic` section header + `HoverClickCreateAutomaticEntryItem` row appear: no icon, Maccy display name in `labelColor`, `(Automatic)` in `secondaryLabelColor` 11 pt smaller secondary text. Maccy is NOT persisted into `excludedAppBundleIDs`; the built-in bypass remains unconditional. Selecting Maccy via `Add Exclusion...` shows "Already Handled Automatically" rather than duplicating it.
...
- **Excluded Apps structural redesign:** `Ignored apps` info row removed. When Maccy is installed, an `Automatic` section header + `HoverClickCreateAutomaticEntryItem` row appear (no icon; `(Automatic)` secondary text in `secondaryLabelColor` 11 pt; `labelColor` for the app name). `Added exclusions` section header replaces the old separator. Empty state shows `No added exclusions`. Bottom action row renamed from `Configure for...` to `Add Exclusion...`. Section headers use new `HoverClickCreateSubmenuSectionHeader(title, width)` function.
- **Contrast improvement:** Maccy `(Automatic)` uses `secondaryLabelColor` instead of `disabledControlTextColor` (the previous "Auto" accessory via a disabled HoverClickMenuRowView); Guide detail uses 65% `labelColor` instead of `secondaryLabelColor`.
...
- `Excluded Apps` submenu (Functions section, after Bypass Key) now shows: optional `Automatic` section header + Maccy automatic row (when Maccy is installed; no icon, `(Automatic)` secondary label, non-actionable), a separator, `Added exclusions` section header, user-added apps by friendly display name (each clickable to remove) sorted A–Z or `No added exclusions`, a separator, then `Add Exclusion...`. The previous `Ignored apps` info row has been removed (`polish-v1.3-menu-ui`).
