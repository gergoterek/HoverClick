...
- Multi-monitor menu bar pass-through: before any AX lookup, HoverClick checks whether the click point falls within the menu bar area of any connected screen (using `NSScreen.visibleFrame` vs `NSScreen.frame`). If it does, the focus attempt is skipped entirely and the original event is returned unchanged. This prevents HoverClick from focusing a background app window that happens to lie under the menu bar area on a secondary display, which was causing the first click on the other display's menu bar to fail.
