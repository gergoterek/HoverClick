...
- Maccy automatic row (#031): `buildExcludedAppsSubmenu` now checks `URLForApplicationWithBundleIdentifier:HoverClickMaccyBundleID` on each menu open. If Maccy is installed, a disabled non-removable row with `checkmark.circle` icon and "Auto" accessory appears first in the list (above user-added apps). Maccy is NOT persisted into `excludedAppBundleIDs`; the built-in bypass remains unconditional. Selecting Maccy via `Configure for...` continues to show the "Already Handled Automatically" alert rather than duplicating it.
