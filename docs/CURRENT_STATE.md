...
- `Excluded Apps` submenu (Functions section, after Bypass Key) now shows: two disabled explanatory rows (`Apps listed here are ignored by HoverClick.` and `Click an app to remove it.`, smaller secondary font, non-clickable), a separator, then user-added apps by friendly display name (each clickable to remove) or `No apps added`, a separator, then `Configure for...`.
