...
- Multi-monitor menu bar pass-through (ultra-fast path): the menu bar check now fires at the earliest possible point in the left/right mouse-down handler — immediately after acquiring the raw CGEvent point, before any `NSWorkspace` IPC, before any `NSMutableDictionary` allocation, and before any diagnostic string formatting. If the click point is in the menu bar strip of any connected screen the original event is returned unchanged with only three cheap writes: `_totalMenuStatusUISkips++`, last-passthrough sequence/time/point (primitive struct assignments). No `beginRecentDecisionWithTrigger:` is called (no `[NSWorkspace sharedWorkspace].frontmostApplication` IPC, no 20-entry NSDictionary allocation), no `diagnosticLog:` string is formatted, no `recordFocusDecisionWithTrigger:` is called, no decision-history entry is created, and no focus attempt is made. The menu bar strip height is determined as the maximum of `NSScreen.visibleFrame`-reported height and `NSStatusBar.systemStatusBar.thickness` (ensuring the check works even when auto-hide is ON). The full check geometry (CGEvent point, AppKit point, matched screen frame/visibleFrame, computed heights, boundary, YES/NO result) is still stored in `_lastMenuBarCheckDescription` and shown as `Last menu-bar/top-strip check:` in Copy Diagnostics Summary. The last ultra-fast passthrough (left/right, sequence, timestamp, raw point) is stored as primitives and formatted lazily at summary-build time, appearing as `Last ultra-fast menu-bar passthrough:` in Copy Diagnostics Summary. Menu-bar-bypassed clicks no longer appear in the Recent non-menu decisions history (no decision entry is created for the fast path).
