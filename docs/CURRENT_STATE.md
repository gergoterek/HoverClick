...
### research-menu-tooltips: Quick Help submenu + Maccy automatic row (2026-06-25)

Branch `research-menu-tooltips` (from `main` at `77de797`, after Excluded Apps merged). Outcome 2 from the plan: native in-menu hover tooltips cannot be made reliably visible on custom `NSView` menu rows, so this branch ships a visible, compact **help fallback** and a **visible Maccy automatic exclusion row** instead of pretending tooltips work.

- Approach chosen: a dedicated **`Guide` submenu** (originally called `Quick Help`; renamed to `Guide` on `polish-v1.3-menu-ui`) in the Info section, placed above the existing `Help` submenu. Native hover tooltips were NOT re-attempted; the reverted `viewDidMoveToWindow` fix (`4fe9e91`) was NOT re-landed, and `ui-menu-tooltips` was neither merged nor used as a base. Re-landing it would reintroduce a fix that still does not display tooltips, because `NSMenu`'s modal tracking loop never drives `NSView` tooltip display (baseline reason #2 above) — closing that gap needs forbidden timers/monitors/floating windows.
...
- Mechanism: `HoverClickCreateQuickHelpItem(title, detail)` builds a disabled, non-actionable `NSMenuItem` whose `view` is a plain `NSView` containing a compact feature label (11 pt medium weight) and a word-wrapped secondary description (11 pt regular, secondaryLabelColor) sized to fit. Vertical padding is 4 pt per edge; title height is 14 pt. It is intentionally NOT a `HoverClickMenuRowView`: no tracking area, no hover highlight, no `mouseDown:` event loop, and it is skipped by `resetMenuRowHighlightsInMenu:`. The submenu is fixed to `HoverClickMenuContentWidth` (286 pt) so descriptions wrap rather than widen; the main menu and the Excluded Apps submenu are unchanged.
- Quick Help rows (8): Left Click Focus, Right Click Focus, Bypass Key, Excluded Apps, Launch at Login, Permissions, Verbose Mode, Copy Summary. Copy uses short, non-technical one-liners (e.g. "Focuses windows on left-click.", "Hold the key to bypass HoverClick.").
- Quick Help visual polish (#031): title font reduced from 13 pt semibold to 11 pt medium; vertical padding reduced from 6 pt to 4 pt; title height reduced from 17 pt to 14 pt; all 8 description strings replaced with compact one-liners. Net visual result: rows are meaningfully shorter and the title no longer reads as a section heading.
- Maccy auto-added row (#031 / polished on #033 / refined on #034): `buildExcludedAppsSubmenu` checks `URLForApplicationWithBundleIdentifier:HoverClickMaccyBundleID` on each menu open. If Maccy is installed, an `Auto-added` section header + `HoverClickCreateAutomaticEntryItem` row appear: `lock` SF Symbol icon in `secondaryLabelColor`, Maccy display name in `secondaryLabelColor` (subtly muted, readable, visually distinct from removable rows), no right-side accessory text. Maccy is NOT persisted into `excludedAppBundleIDs`; the built-in bypass remains unconditional. Selecting Maccy via `Add App Exclusion...` shows "Already Handled Automatically" rather than duplicating it.
...
- Status: NOT merge-ready and NOT release-ready until the user manually confirms Quick Help compactness, Maccy automatic row behavior, Excluded Apps behavior, A–Z sorting, selector/remove/persistence, and runtime click behavior. Scripts passing is not sufficient.
- Merged to main at `2bf8acb` (2026-06-25, #032). User manually confirmed: "minden jó, mehet a merge".

### v1.3 Menu UI Polish: polish-v1.3-menu-ui (2026-06-25, #033)

Branch `polish-v1.3-menu-ui` (from `main@2bf8acb`). Final menu UI polish before v1.3 release prep. Changes:
- **Guide (renamed from Quick Help):** `Quick Help` submenu renamed to `Guide` everywhere (title, submenu title, comment). Detail text color improved from `secondaryLabelColor` to `[[NSColor labelColor] colorWithAlphaComponent:0.65]` for slightly better legibility.
- **Excluded Apps structural redesign (#033):** `Ignored apps` info row removed. `Auto-added` section header + `HoverClickCreateAutomaticEntryItem` row when Maccy is installed. `Added exclusions` section header. Empty state shows `No added exclusions`. Bottom action row renamed `Add App Exclusion...`. Section headers via `HoverClickCreateSubmenuSectionHeader(title, width)`.
- **Maccy row refinement (#034):** `Auto-added` (renamed from `Automatic`). `lock` SF Symbol icon in `secondaryLabelColor`. App name in `secondaryLabelColor` (subtly muted, readable). No right-side `(Automatic)` accessory text removed. `Add App Exclusion...` (renamed from `Add Exclusion...`).
- **Contrast improvement:** Guide detail uses 65% `labelColor` instead of `secondaryLabelColor`; Maccy row uses `secondaryLabelColor` for both icon and name.
- **Runtime/safety:** event tap mask unchanged; no timers, monitors, synthetic clicks, event replay, cursor movement; no floating windows; no version/build bump.
- Status: NOT merge-ready. Manual testing required — see `docs/TEST_MATRIX.md` section "Menu UI Polish (polish-v1.3-menu-ui)" and "Menu Guide + Maccy Automatic Row".
...
- `Excluded Apps` submenu (Functions section, after Bypass Key) now shows: optional `Auto-added` section header + Maccy auto-added row (when Maccy is installed; lock icon + muted name in `secondaryLabelColor`; no right-side accessory; non-actionable), a separator, `Added exclusions` section header, user-added apps by friendly display name (each clickable to remove) sorted A–Z or `No added exclusions`, a separator, then `Add App Exclusion...`. The previous `Ignored apps` info row has been removed (`polish-v1.3-menu-ui`).
