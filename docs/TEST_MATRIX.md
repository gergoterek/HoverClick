...
| Header row appears | Manual Finder UI validation — not run automatically. Open the HoverClick status menu. | `HoverClick` and `v0.4.2 (21)` appear on the same non-clickable header row, with the version visually separated on the right. |
| Header tooltip appears | Manual Finder UI validation — not run automatically. Hover over the header row. | Tooltip identifies the running HoverClick version. |
| Visible version follows Info.plist | Inspect `Info.plist`, build, relaunch through `scripts/run-app.sh`, then open the menu manually. | The header shows `v<CFBundleShortVersionString> (<CFBundleVersion>)` from the built app metadata. |
...
| Diagnostics submenu appears | Manual Finder UI validation — not run automatically. Open the HoverClick status menu. | `Diagnostics` appears as a submenu containing only `Verbose Diagnostics` and `Copy Diagnostics Summary`. |
...
| Accessibility status is detected correctly | Use the status menu permission item before and after granting permission. | The submenu item reports `Accessibility: Granted` or `Accessibility: Not Granted`, shows a checkmark when granted, and logs report trusted `YES` or `NO`. |
...
| Phase 1 event tap installs when allowed | Grant Accessibility, then run `scripts/run-app.sh`. | Logs include `[HoverClick] event tap installed mode=pass-through-default`; click detection status is available through `Copy Diagnostics Summary`. |
...
| Copy Diagnostics Summary works | Manual Finder UI validation — not run automatically. Click `Diagnostics` > `Copy Diagnostics Summary`, then paste into a text field. | The copied text includes version/build, Accessibility, Launch at Login, click detection, last handled action, feature states, event tap mask, and stable-core safety details. |
...
| Open Accessibility Settings is explicit | Manual Finder UI validation — not run automatically. Open `Permissions & Startup` without clicking the settings item. | System Settings does not open unless `Open Accessibility Settings` is clicked explicitly. |
...
| Trigger controls stay separate | Inspect menus and source. | Current top-level feature controls include Left Click Focus and Right Click Focus; `Hover Click Assist` lives under `Hover`; technical click detection appears only in the copied diagnostics summary; Scroll Focus remains future work. |
